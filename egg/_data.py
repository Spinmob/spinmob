...
class fitter():
    ...
    def set_data(self, xdata=[1,2,3,4,5], ydata=[[1,2,1,2,1],[3,2,3,4,3]], eydata=None):
        ...
        if not _is_iterable(eydata):    eydata = [eydata]
        if not _is_iterable(eydata[0]): eydata = [eydata]
    ...
    def _massage_data(self):
        ...
        for n in range(len(self.xdata)):
            ...
            or (not self['xmax'][n]==None and self['xmax'][n] < min(self.xdata[n])):
                ...
                x, y, ey = trim_data(None, None,
                                     self.xdata[n], self.ydata[n], self.eydata[n])
            else:
                x, y, ey = trim_data(self['xmin'][n], self['xmax'][n],
                                     self.xdata[n], self.ydata[n], self.eydata[n])
    ...
    def autoscale_eydata_and_fit(self):
        ...
        """
        if not self.results: self._error("You must complete a fit first.")
